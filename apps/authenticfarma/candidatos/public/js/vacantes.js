...
// document.addEventListener('DOMContentLoaded', () => {
    ...
    // Función para restaurar todos los filtros
    function restaurarFiltros() {
        // Limpiar el objeto filtros
        Object.keys(filtros).forEach(key => {
            filtros[key] = '';
        });

        // Limpiar todos los campos de filtros en el DOM
        ...
        // Restablecer la página actual
        paginaActual = 1;

        // Actualizar chips y aplicar filtros
        actualizarChips();
        aplicarFiltros();

        // Restablecer el texto descriptivo
        document.querySelector('.text-muted i').textContent = 'Encuentra tu próxima oportunidad laboral';
    }
    ...
    ['ciudad', 'sector', 'area', 'nivel', 'salario'].forEach(filtro => {
        document.getElementById(`filtro-${filtro}`).addEventListener('change', (e) => {
            // Map 'nivel' to 'nivel_educacion' in the filtros object
    ...
    function aplicarAVacante(id, cargo) {
        try {
            ...
            const form = document.getElementById('formAplicar');
            if (!form) {
                throw new Error('Formulario no encontrado');
            }

            const textoCargo = modal.querySelector('.texto-cargo');
            if (!textoCargo) {
                throw new Error('Elemento texto-cargo no encontrado');
            }

            const idInput = document.getElementById('idofoferta_laboral');
            if (!idInput) {
                throw new Error('Input de ID no encontrado');
            }

            // Limpiar y configurar el formulario
            form.reset();
            form.action = `/vacante/${id}/aplicar`;
            idInput.value = id;
            ...
            console.error('Error en aplicarAVacante:', error);
            alert('Hubo un problema al mostrar el formulario de aplicación. Por favor, intente de nuevo. Error: ' + error.message);
        }
    }
    ...
    document.getElementById('modalAplicar').addEventListener('click', function(e) {
        if (e.target === this) {
            cerrarModal();
        }
    });
    ...
    document.querySelector('.app-modal-content').addEventListener('click', function(e) {
        e.stopPropagation();
    });

    // Hacer funciones disponibles globalmente
    window.restaurarFiltros = restaurarFiltros;
    window.aplicarAVacante = aplicarAVacante;
