<form id="formAgregarEducacion" method="POST" action="{{ route('educacion.store') }}" >
    @csrf
            ...
            <div class="col-md-6">
                <div class="form-floating-group">
                    <select name="nivel_educacion" required>
                        <option value="" disabled {{ old('nivel_educacion') ? '' : 'selected' }}></option>
                        @foreach ($nivelesEducacion as $nivel)
                            <option value="{{ $nivel->id }}" {{ old('nivel_educacion') == $nivel->id ? 'selected' : '' }}>
                                {{ $nivel->nombre }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="col-md-6">
                <div class="form-floating-group">
                    <select name="pais" required>
                        <option value="" disabled {{ old('pais', $educacion->pais_id ?? '') ? '' : 'selected' }}></option>
                        @foreach ($paises as $pais)
                            <option value="{{ $pais->id }}" {{ old('pais', $educacion->pais_id ?? '') == $pais->id ? 'selected' : '' }}>
                                {{ $pais->nombre }}
                            </option>
                        @endforeach
                    </select>
