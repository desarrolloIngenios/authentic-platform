<form id="formEditarEducacion" method="POST" action="{{ route('educacion.update', $registro->idhvcan_form_ac) }}" novalidate>
    @csrf
    @method('PUT')
    <div class="custom-modal-body">
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-floating-group">
                    <select name="nivel_educacion" required>
                        <option value="" disabled {{ !$registro->nivel_educacion ? 'selected' : '' }}></option>
                        @foreach ($nivelesEducacion as $nivel)
                            <option value="{{ $nivel->id }}" {{ $registro->id_nivel_educacion === $nivel->id ? 'selected' : '' }}>
                                {{ $nivel->nombre }}
                            </option>
                        @endforeach
                    </select>
                    <span class="floating-label">Nivel de educación *</span>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-floating-group">
            ...
            <div class="col-md-6">
                <div class="form-floating-group">
                    <select name="pais" required>
                        <option value="" disabled {{ !$registro->id_pais ? 'selected' : '' }}></option>
                        @foreach ($paises as $pais)
                            <option value="{{ $pais->id }}" {{ $registro->id_pais == $pais->id ? 'selected' : '' }}>
                                {{ $pais->nombre }}
                            </option>
                        @endforeach
                    </select>
